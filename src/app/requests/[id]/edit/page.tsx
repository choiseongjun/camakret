...
export default function EditRequestPage() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({
          ...
          channel_subscribers: request.channel_subscribers || '',
          category: request.category || categories[0],
          guest_type: request.guest_type || '',
          content_concept: request.content_concept || '',
          shooting_location: request.shooting_location || '',
          preferred_date: request.preferred_date || '',
          fee_range: request.fee_range || '',
          fee_negotiable: request.fee_negotiable ?? true
        });
      }
    } catch (error) {
      console.error('요청글 조회 에러:', error);
      alert('요청글을 불러오는 중 오류가 발생했습니다.');
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        alert('협업 요청글이 수정되었습니다!');
        router.push(`/requests/${params.id}`);
      } else {
        alert('수정 중 오류가 발생했습니다: ' + result.message);
      }
    } catch (error) {
      console.error('요청글 수정 에러:', error);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50">
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">협업 요청글 수정</h1>
